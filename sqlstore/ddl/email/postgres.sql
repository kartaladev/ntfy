...
CREATE TABLE IF NOT EXISTS "{{PREFIX}}notify_email_deliveries" (
    ...
    CONSTRAINT "{{PREFIX}}notify_email_deliveries_pkey" PRIMARY KEY ("notification_id")
);

-- Finding leases that lapsed.
CREATE INDEX IF NOT EXISTS "{{PREFIX}}notify_email_deliveries_lease_idx"
    ON "{{PREFIX}}notify_email_deliveries" ("status", "lease_until");

-- Finding retries that are due.
CREATE INDEX IF NOT EXISTS "{{PREFIX}}notify_email_deliveries_retry_idx"
    ON "{{PREFIX}}notify_email_deliveries" ("status", "next_attempt_at");

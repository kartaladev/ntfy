...
CREATE TABLE IF NOT EXISTS `ntfy_notifications` (
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `ntfy_notifications_source_key` (`source_id`, `recipient`),
    KEY `ntfy_notifications_recipient_idx` (`recipient`, `created_at`, `id`),
    KEY `ntfy_notifications_state_idx` (`recipient`, `state`),
    KEY `ntfy_notifications_subject_idx` (`subject`, `kind`, `state`),
    KEY `ntfy_notifications_inactive_idx` (`state`, `inactive_at`)
...
CREATE TABLE IF NOT EXISTS `ntfy_watermarks` (
    ...
    KEY `ntfy_watermarks_updated_idx` (`updated_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

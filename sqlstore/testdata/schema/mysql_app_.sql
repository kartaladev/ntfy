...
CREATE TABLE IF NOT EXISTS `app_notify_notifications` (
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `app_notify_notifications_source_key` (`source_id`, `recipient`),
    KEY `app_notify_notifications_recipient_idx` (`recipient`, `created_at`, `id`),
    KEY `app_notify_notifications_state_idx` (`recipient`, `state`),
    KEY `app_notify_notifications_subject_idx` (`subject`, `kind`, `state`),
    KEY `app_notify_notifications_inactive_idx` (`state`, `inactive_at`)
...
CREATE TABLE IF NOT EXISTS `app_notify_watermarks` (
    ...
    KEY `app_notify_watermarks_updated_idx` (`updated_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
